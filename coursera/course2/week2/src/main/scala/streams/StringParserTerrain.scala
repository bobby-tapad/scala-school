...
trait StringParserTerrain extends GameDef {
  ...
  def terrainFunction(levelVector: Vector[Vector[Char]]): Pos => Boolean = p => {
    if (levelVector.indices.contains(p.row) &&
      levelVector(p.row).indices.contains(p.col) &&
      levelVector(p.row)(p.col) != '-') true
    else false
  }
  ...
  def findChar(c: Char, levelVector: Vector[Vector[Char]]): Pos = (for {
    ...
    if terrainFunction(levelVector)(Pos(row, col)) && (levelVector(row)(col) == c)
  } yield Pos(row, col))(0)

...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter(n => !explored.contains(n._1))
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from((startBlock, List[Move]()) #:: LazyList[(Block, List[Move])](), Set())

  /**
   * Returns a lazy list of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(x => (x._1.b1 == goal) && (x._1.b2 == goal))

...
class CodecsSuite
  ...
  @Test def `a 'Boolean' true value should be encoded as a JSON string (1pt)`(): Unit = {
    assert(implicitly[Encoder[Boolean]].encode(true) == Json.Bool(true))
  }

  @Test def `a 'Boolean' false value should be encoded as a JSON string (1pt)`(): Unit = {
    assert(implicitly[Encoder[Boolean]].encode(false) == Json.Bool(false))
  }

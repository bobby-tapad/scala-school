...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] =
    Encoder.fromFunction(n => Json.Str(n.toString))
...
trait DecoderInstances {
  ...
  /** A decoder for `Int` values. Hint: use the `isValidInt` method of `BigDecimal`. */
    // why do i need to use a named var 'x' here instead of _?
  implicit val intDecoder: Decoder[Int] = Decoder.fromPartialFunction { case Json.Num(x) if x.isValidInt => x.toInt }

  /** A decoder for `String` values */
  implicit val stringDecoder: Decoder[String] = Decoder.fromFunction { x => Option(x.asInstanceOf[Json.Str].value)}

  /** A decoder for `Boolean` values */
  implicit val boolDecoder: Decoder[Boolean] = Decoder.fromFunction { x => {
      val b = if (x.toString.toLowerCase.contains("true")) true else false
      Option(b)
    }
  }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      as => {
        val list = as.asInstanceOf[Json.Arr].items.map(_.decodeAs[A] match {
          case Some(x) => x
      ...
      //      as => {
//        val list: List[A] = for (a <- as.asInstanceOf[Json.Arr].items ) yield a.asInstanceOf[A]
//        Option(list)
//      }
    }
  ...
  /**
    ...
    */
    def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] = {
      Decoder.fromFunction {
        os => {
          os.asInstanceOf[Json.Obj].fields(name).decodeAs[A] match {
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] = Decoder.field[String]("name").
    zip(Decoder.field[Int]("age")).transform[Person]( x => Person(x._1, x._2) )
}
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] = ObjectEncoder.field[List[Person]]("people").
    transform[Contacts]{ p => p.people }

  implicit val contactsDecoder: Decoder[Contacts] = Decoder.field[List[Person]]("people").
    transform[Contacts]{ p => Contacts(p)}
}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(renderJson(true))

    val maybeJsonString = parseJson(""" "foo" """)
    val maybeJsonObj    = parseJson(""" { "name": "Alice", "age": 42 } """)
    val maybeJsonObj2   = parseJson(""" { "name": "Alice", "age": "42" } """)

    val maybeJsonTrue = parseJson( """ true """)
    val maybeJsonFalse = parseJson( """ false """)
    // Uncomment the following lines as you progress in the assignment
    println(maybeJsonTrue.flatMap(_.decodeAs[Boolean]))
    println(maybeJsonFalse.flatMap(_.decodeAs[Boolean]))

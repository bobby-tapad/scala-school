 ...
 */
  ...
  def wordOccurrences2(w: Word): Occurrences = w.toLowerCase.toList.groupBy(identity).toList.map(x => (x._1, x._2.size)).sorted // :(
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      case List() => List(List())
      case x :: xs => {
        val sub: List[Occurrences] = combinations(xs)
        val acc = for {
          a <- sub
          i <- 1 to x._2
        } yield (x._1, i) :: a
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y match {
      case Nil => x
      case y :: ys => {
        val matched: (Char, Int) = x.filter(z => z._1 == y._1).head
        val missed = x.filterNot(z => z._1 == y._1)
        val total = matched._2 - y._2
        if (total > 0) (matched._1, total) :: subtract(missed, ys)
        else subtract(missed, ys)
      }
    }).sortBy(_._1)
  }
